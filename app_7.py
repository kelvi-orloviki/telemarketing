...
def main():
    ...
    st.set_page_config(page_title = 'Telemarketing analisys', \
        page_icon = 'C:/Users/Kelvi Orloviki/Desktop/EBAC/Profissão Cientista de Dados/Cientista de Dados/Módulo 19 - (Desenvolvimento Modelos com Pandas e Python) - Streamlit II/Tarefa 1/img/telmarketing_icon.png',
        layout="wide",
    ...
    if (data_file_1 is not None):
        ...
        # Corrige os DataFrames para conter colunas explícitas para gráficos
        ...
        try:
            ...
            st.error('Erro ao aplicar os filtros nos dados.')
            return  # Encerra a função main caso o filtro falhe
        ...
        col1.download_button(label='📥 Download', data=df_xlsx, file_name='bank_raw_y.xlsx')

        df_xlsx = to_excel(bank_target_perc)
        col2.write('### Proporção da tabela com filtros')
        col2.write(bank_target_perc)
        col2.download_button(label='📥 Download', data=df_xlsx, file_name='bank_y.xlsx')
        ...
        if graph_type == 'Barras':
            sns.barplot(x='target', y='y', data=bank_raw_target_perc, ax=ax[0], palette=['#1f77b4', '#ff7f0e'])
            ax[0].bar_label(ax[0].containers[0])
            ax[0].set_title('Dados brutos', fontweight="bold")

            sns.barplot(x='target', y='y', data=bank_target_perc, ax=ax[1], palette=['#1f77b4', '#ff7f0e'])

            ax[1].bar_label(ax[1].containers[0])
            ax[1].set_title('Dados filtrados', fontweight="bold")
        else:
            bank_raw_target_perc.set_index('target').plot(kind='pie', y='y', autopct='%.2f%%', ax=ax[0])
            ax[0].set_ylabel('')  # Remove o label "y"
            ax[0].set_title('Dados brutos', fontweight="bold")

            bank_target_perc.set_index('target').plot(kind='pie', y='y', autopct='%.2f%%', ax=ax[1])
            ax[1].set_ylabel('')
            ax[1].set_title('Dados filtrados', fontweight="bold")
